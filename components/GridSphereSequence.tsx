...
export default function GridSphereSequence({
  ...
  // Size the canvas buffer to match the wrapper's real aspect ratio,
  // and keep it in sync on resize/orientation change (fixes mobile too)
  useEffect(() => {
    const canvas = canvasRef.current;
    const wrapper = canvas?.parentElement;
    if (!canvas || !wrapper || !isReady) return;
    ...
    const render = () => {
      ...
      if (img && img.complete) {
        ...
        if (imgRatio > canvasRatio) {
          // source wider than target box -> crop left/right
          sw = sourceHeight * canvasRatio;
          sx = (sourceWidth - sw) / 2;
        } else {
          // source taller than target box -> crop top/bottom
  ...
  return (
    <div ref={containerRef} className="relative h-[500vh] bg-pearl" style={{ position: "relative" }}>
      <div className="sticky top-0 h-screen w-full overflow-hidden flex items-start md:items-center justify-center pt-8 md:pt-0 relative bg-pearl">

        {/* Sequence canvas wrapper - controls visible height without cropping the sides */}
        <div className="relative w-full" style={{ height: "78vh", overflow: "hidden" }}>
          <canvas
            ref={canvasRef}
        ...
        <div className="absolute inset-0 pointer-events-none z-20" style={{ background: 'radial-gradient(ellipse 70% 75% at center, transparent 40%, rgba(244,239,230,0.5) 70%, #F4EFE6 100%)' }} />
        <div className="absolute inset-0 pointer-events-none z-20" style={{ background: 'linear-gradient(to right, #F4EFE6 0%, #F4EFE6 20%, transparent 21%, transparent 79%, #F4EFE6 80%, #F4EFE6 100%)' }} />
        <div className="absolute inset-0 pointer-events-none z-20" style={{ background: 'linear-gradient(to bottom, #F4EFE6 0%, #F4EFE6 8%, transparent 10%, transparent 90%, #F4EFE6 92%, #F4EFE6 100%)' }} />
